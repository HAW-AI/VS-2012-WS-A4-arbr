...
slot_received({ slot, Slot }, State) ->
  ...
  { next_state, message_received, State#state{ slot=Slot, next_slot=Slot } };
...
%      timer:sleep(TimeLeft),
      ...
      { next_state, deliver, State#state{message=Message} };
    false ->
      %log("[~p]TimeLeft<=0",[State#state.station]),
      { next_state, slot_received, State#state{message=Message} }
  end;
